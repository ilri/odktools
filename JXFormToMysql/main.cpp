...
//    UUIDStrm << "-- Code generated by JXFormToMySQL" << "\n";
//    UUIDStrm << "-- " + command << "\n";
//    UUIDStrm << "-- Created: " + date.toString("ddd MMMM d yyyy h:m:s ap")  << "\n";
//    UUIDStrm << "-- by: JXFormToMySQL Version 1.0" << "\n";
//    UUIDStrm << "-- WARNING! All changes made in this file might be lost when running JXFormToMySQL again" << "\n\n";
...
void parseOSMField(TtableDef &OSMTable, QJsonObject fieldObject)
{
    ...
    if (fieldObject.keys().indexOf("choices") >= 0)
    {
        ...
        aField.formula = "";
        aField.calculateWithSelect = false;
        aField.xmlCode = variableName.toLower();
...
void parseField(QJsonObject fieldObject, QString mainTable, QString mainField, QDir dir, QSqlDatabase database)
{
    ...
    QString variableName;
    variableName = fieldObject.value("name").toString();
    variableName = variableName.toLower().trimmed();
    ...
    if (isSelect(variableType) == 0)
    {
        ...
        if (getVariableStack(false) != "")
        {
            aField.xmlCode = getVariableStack(false) + "/" + variableName;
...
void parseTable(QJsonObject tableObject, QString tableType)
{
    ...
    {
        aTable.xmlCode = getVariableStack(false) + "/" + variableName;
        aTable.xmlFullPath = getVariableStack(false) + "/" + variableName;
    }
    ...
    if (tableType == "osm")
    {
        if (getVariableStack(false) != "")
        {
            aTable.xmlCode = getVariableStack(false) + "/" + aTable.name;
        }
        else
        {
            aTable.xmlCode = variableName;
...
int processJSON(QString inputFile, QString mainTable, QString mainField, QDir dir, QSqlDatabase database)
{
    ...
    if (!firstObject.isEmpty())
    {
        ...
        {
            if (surveyVariables[ivar].name == mainField)
            {
                mainFieldIndex = ivar;
                break;
            }
        ...
        parseField(primaryKeyObject,mainTable, mainField, dir, database);
