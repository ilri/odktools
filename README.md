...
ODKToMySQL converts a ODK Excel File (XLSX survey file) into a relational MySQL schema. Even though [ODK Aggregate](https://opendatakit.org/use/aggregate/) stores submissions in MySQL, the Aggregate schema lacks basic functionality like:
 ...
 - manifest.xml. This file maps each variable in the ODK survey with its corresponding field in a table in the MySQL database. **This file is used in subsequent tools.**
  ...
  - I - Output lookup tables and values in XML format. Used by utilities compareInsertXML & insertFromXML.
  - C - Output schema as in XML format. Used by utilities compareCreateXML & createFromXML
...
#### *Parameters*
  ...
  - J - Input JavaScript file with a beforeInsert Function to customize the values entering the database
...
The insert process of JSONToMySQL can be hooked up to an external JavaScript file to perform changes in the data before insert them in the MySQL database. This is done by creating a .js file with the following format:
...
Parameters:
table: a JavaScript string variable. Contains the table being processed
data: a   JavaScript object. Contains a row of data.

The “data: object has the following structure:
Properties:
- int  count: contains the number if items (columns) in the row.
Read functions:
- String itemName(int  index): Returns the column name for an item index.
- String itemXMLCode(int  index):  Returns the ODK column name for an item index.
- String itemValue(int  index):  Returns the column value as string for an item index.
-bool itemIsKey(int  index): Returns whether the column is key for an item index.
- bool itemToInsert(int  index): Returns whether the column is going to be inserted for an item index.
- bool ItemIsMultiSelect(int  index): Returns whether the column is a multiselect for an item index.
- String itemMultiSelectTable(int  index): Returns the multiselect table for an item index.
Set functions:
- insertValue(int name, String xmlCode, String value, bool isKey, bool  isMultiSelect, String multiSelectTable): Inserts a new column.
- setItemName(int index, String name): Change the name of a column.
- setItemXMLCode(int index, String xmlCode): Change the ODK name of a column.
- setItemValue(int index, String value): Change the value of a column.
- setItemIsKey(int index, bool isKey): Change a column to be key or not.
- setItemIsMultiSelect(int index, bool isMultiSelect): Change a column to be multiselect or not.
- setItemMultiSelectTable(int index, String table): Change the multiselect table of a column.
- setItemToInsert(int index, bool toInsert): Change a column to be inserted or not. Note: It does has an effect on key columns.
Utility functions:
- int getIndexByColumnName(String name): Return the index of a column by its name. Notes: This is NOT case sensitive.
- bool valueIsNumber(int index): Returns whether the value of a column is a number or not.
